...
apps/website/            Marketing site (React + TS, prerendered, EN/DE)
...
npm run build --workspace @blame-game/website
npm run preview --workspace @blame-game/website   # builds, serves dist/ on :4173
```
- `apps/website` is React + TypeScript, prerendered by `src/entry-static.tsx`
  and hydrated by `src/entry-client.tsx`. The HTML is complete before any script
  runs; the client bundle only adds motion. `<html class="no-js">` plus an
  inline unsetter means a no-JS visitor still sees everything that would
  otherwise animate in.
- Animated pieces are vendored from the React Bits registry into
  `src/components/reactbits/` and edited to pass this repo's strict tsconfig —
  **re-fetching a component overwrites those edits**.
- `public/screenshots/` are real simulator captures, not mockups. Regenerate
  with `scripts/capture-app-screenshots.sh` then `scripts/prepare-screenshots.sh`.
...
- `apps/website/public/` is generated by `scripts/generate-app-icons.sh`, not
  hand-made. See the branding section above.
- The copy says the app is out on the App Store. **It is not** — the store
  button's href is deliberately empty (`stores.appStore` in `packages/brand`).
  The page must not go public until that is true and the link is filled in.
