...
```
- `apps/website` is React + TypeScript but ships **no client JavaScript**: the
  language switch is links, the FAQ is `<details>`, and `src/entry-static.tsx`
  renders each locale with `renderToStaticMarkup`. The HTML a crawler sees is
  the HTML a person sees.
